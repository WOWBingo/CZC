...
-(id)initWithPoint:(CGPoint)point titles:(NSMutableArray *)titles images:(NSArray *)images
{
    self = [super init];
    if (self) {
...
-(CGRect)getViewFrame
{
    ...
    for (NSString *typeStr in self.titleArray) {
        CGFloat width =  [typeStr sizeWithFont:TITLE_FONT constrainedToSize:CGSizeMake(300, 100) lineBreakMode:NSLineBreakByCharWrapping].width;
...
-(void)show
{
    ...
    [_handerView setBackgroundColor:[UIColor clearColor]];//全屏颜色
...
{
    //定义广播
    [[NSNotificationCenter defaultCenter] postNotificationName:@"dissPop" object:nil];
    [self dismiss:YES];
}
...
//    }
    ...
    if ([titleStr isEqualToString:self.titleName]) {
        cell.textLabel.textColor = RGB(140, 199, 63, 1);
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.selectRowAtIndex) {
        ...
        NSString *typeName = [self.titleArray objectAtIndex:[indexPath row]];
        NSString *typeTag = [NSString stringWithFormat:@"%ld",(long)indexPath.row];
        //定义广播数据，其类型为NSDisdictionary
        NSDictionary *dicData = [[NSDictionary alloc]initWithObjectsAndKeys:typeTag,@"typeTag",typeName,@"typeName", nil];
        //定义广播
        [[NSNotificationCenter defaultCenter] postNotificationName:@"infoTypeTag" //广播名称
                                                            object:nil userInfo:dicData];
    }
    [self dismiss:YES];
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    //填充颜色
//    [RGB(245, 0, 245) setFill];
    [[UIColor colorWithRed:128/255 green:128/255 blue:128/255 alpha:0.5] setFill];
    [popoverPath fill];
